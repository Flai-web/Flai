...
const ProductPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Wait until products are both loaded AND populated — isProductsLoaded can
    // become true before the products array is filled, causing an immediate
    // not-found redirect on the first render.
    if (!isProductsLoaded || products.length === 0) {
      return;
    }

    // React Router v6 may or may not pre-decode path params depending on version —
    // wrap in try/catch so a malformed % sequence never crashes the page.
