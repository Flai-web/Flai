...
const Section_9009b281 = ((
  ...
  const Testimonials: React.FC = () => {
    ...
    // Auto-advance — only when there are enough reviews to navigate on that breakpoint
    // Desktop: needs > 2; Mobile: needs > 1. We always auto-advance if mobile condition met.
    useEffect(() => {
    ...
    // Desktop (≥769px): show only when reviews.length > 2
    // Mobile (<769px):  show only when reviews.length > 1
    ...
    return (
      <section className="py-20">
        <style>{`
          ...
          /* Divider + inner gutter between the two cards on desktop */
          ...
          /* ── Nav visibility per breakpoint ── */
          .testi-nav-row {
            /* Hidden by default; shown conditionally via JS-driven classes below */
          }
          /* Desktop: hide nav if only 1 or 2 reviews */
          @media (min-width: 769px) {
            .testi-nav-row--hide-desktop {
              display: none !important;
            }
          }
          /* Mobile: hide nav if only 1 review */
          @media (max-width: 768px) {
        ...
        <div className="w-full max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="testi-outer">
            ...
            {/* ── Navigation below, full width ──
                Always rendered in DOM; hidden via CSS classes based on counts.
                - Desktop (≥769px): visible only when reviews.length > 2
                - Mobile  (<769px): visible only when reviews.length > 1
            ── */}
            <div
              className={[
                'testi-nav-row',
            ...
            {/* ── Progress bar — mirrors nav visibility ── */}
            {(showNavDesktop || showNavMobile) && (
