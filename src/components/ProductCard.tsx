...
const SWIPE_THRESHOLD = 40; // minimum px to register as a swipe
...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  const handleViewProduct = () => {
    // Don't navigate if we just finished a swipe
  ...
  const isCurrentYoutube = product.images[currentImageIndex]?.startsWith('youtube:');
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (isCurrentYoutube || !hasMultipleImages) return;
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    // Only trigger if more horizontal than vertical movement
    ...
    // Reset isSwiping after a short delay so the click handler can check it
    setTimeout(() => {
      isSwiping.current = false;
    }, 100);
  };
